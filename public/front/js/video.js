...
function playPause(ids){
    $('body').addClass('popup');
    var div = `video-${ids}`;
    var video = document.getElementById(`youtube-${ids}`).src;
    document.getElementById(`youtube-${ids}`).src = video+'&autoplay=1'; // adding autoplay to the URL
    document.getElementById(div).style.display = 'block'
    return;
    // var ids = $(this).attr('attr');
    const video_player = document.querySelector(`.video_player_${ids}`),
  mainVideo = video_player.querySelector(`.main-video-${ids}`),
...
// let thumbnail = video_player.querySelector(".thumbnail"); play_pause
if (tracks.length != 0) {
...
function hideVideo(div,video_id,ids) {
    $('body').removeClass('popup');
    var video = document.getElementById(video_id+ids).src;
    var cleaned = video.replace('&autoplay=1',''); // removing autoplay form url
    document.getElementById(video_id+ids).src = cleaned;
    document.getElementById(div+ids).style.display = 'none';
    $(`#show_video_${ids}`).css("display", "none");
    $(`#final-vedio-${ids}`).css("display", "block");
    // alert(ids)
